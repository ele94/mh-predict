...
def main():
    ...
    with open('data/pickles/train.df.feats.pkl', 'rb') as train_df_feats_file:
        trainDF = pickle.load(train_df_feats_file)

    with open('data/pickles/test.df.feats.pkl', 'rb') as test_df_feats_file:
        testDF = pickle.load(test_df_feats_file)
    ...
    for user in train_users:
        ...
        feats['user'] = user["user"]
        #feats['g_truth'] = user["g_truth"]
    ...
    train_new_DF = pd.DataFrame(train_feats)
    train_new_DF.set_index(['user'])
    test_new_DF = pd.DataFrame(test_feats)
    test_new_DF.set_index(['user'])

    trainDF.merge(train_new_DF, left_on='user', right_on='user')
    testDF.merge(test_new_DF, left_on='user', right_on='user')


    with open('data/pickles/test.feats.mod.pkl', 'wb') as test_feats_file:
        pickle.dump(testDF, test_feats_file)

    with open('data/pickles/train.feats.mod.pkl', 'wb') as train_feats_file:
        pickle.dump(trainDF, train_feats_file)


    import csv
    keys = train_feats[0].keys()
    with open('data/train_feats.csv', 'w', newline='')  as output_file:
        dict_writer = csv.DictWriter(output_file, keys)
        dict_writer.writeheader()
        dict_writer.writerows(train_feats)

    keys = test_feats[0].keys()
    with open('data/test_feats.csv', 'w', newline='')  as output_file:
        dict_writer = csv.DictWriter(output_file, keys)
        dict_writer.writeheader()
        dict_writer.writerows(test_feats)
